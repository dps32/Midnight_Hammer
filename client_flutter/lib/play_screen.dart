...
class PlayScreen extends ScreenAdapter {
  ...
  static const double crosshairSize = 8;
  ...
  static final ui.Color hotbarSelected = colorValueOf('FFE07A');
  ...
  void _renderPlayers(
    ...
    for (final MultiplayerPlayer player in orderedPlayers) {
      ...
      if (!player.alive) {
        batch.setColor(0.7, 0.7, 0.7, 0.45);
  ...
  void _renderLeaderboard(AppData appData) {
    ...
    _drawLeftAlignedText(
      batch,
      font,
      'Alive players: ${appData.alivePlayers}',
      screenWidth - leaderboardWidth + leaderboardPadding,
      64,
      1.0,
      dimTextColor,
  ...
  void _handleCombatInput(AppData appData) {
    final MultiplayerPlayer? local = appData.localPlayer;
    if (local == null || !local.alive) {
      return;
    }

    final ui.Offset aimWorld = _screenToWorld(
      Gdx.input.getX().toDouble(),
      Gdx.input.getY().toDouble(),
    );
    _crosshairWorldX = aimWorld.dx;
    _crosshairWorldY = aimWorld.dy;
    ...
    if (Gdx.input.isKeyJustPressed(Input.keys.num1)) {
      appData.selectSlot(0);
    }
    if (Gdx.input.isKeyJustPressed(Input.keys.num2)) {
      appData.selectSlot(1);
    }
    if (Gdx.input.isKeyJustPressed(Input.keys.num3)) {
      appData.selectSlot(2);
    }
    if (Gdx.input.isKeyJustPressed(Input.keys.num4)) {
      appData.selectSlot(3);
    }
  ...
  ui.Offset _screenToWorld(double screenX, double screenY) {
    final double viewW = math.max(1, viewport.worldWidth * camera.zoom);
  ...
  void _renderProjectiles(List<ProjectileSnapshot> projectiles) {
    if (projectiles.isEmpty) {
      return;
    }
    final ShapeRenderer shapes = game.getShapeRenderer();
    shapes.begin(ShapeType.filled);
    for (final ProjectileSnapshot projectile in projectiles) {
      final ui.Rect dst = viewport.worldToScreenRect(
      ...
      shapes.circle(
        dst.left + dst.width * 0.5,
        dst.top + dst.height * 0.5,
        math.max(1, dst.width * 0.5),
        12,
      );
    }
    shapes.end();
  ...
  void _renderCrosshair() {
    ...
    final double cx = dst.left + dst.width * 0.5;
    final double cy = dst.top + dst.height * 0.5;
    final ShapeRenderer shapes = game.getShapeRenderer();
    shapes.begin(ShapeType.line);
    shapes.setColor(localPlayerColor);
    shapes.line(cx - crosshairSize, cy, cx - 2, cy);
    shapes.line(cx + 2, cy, cx + crosshairSize, cy);
    shapes.line(cx, cy - crosshairSize, cx, cy - 2);
    shapes.line(cx, cy + 2, cx, cy + crosshairSize);
    shapes.end();
  }

...
import 'training_ground_screen.dart';
...
class _GameRootState extends State<_GameRoot> {
  ...
  bool _startInPracticeMode = false;

  void _handleStartGame(NetworkConfig config, bool startInPracticeMode) {
    setState(() {
      _networkConfig = config;
      _startInPracticeMode = startInPracticeMode;
    });
  }
      ...
      home: Scaffold(
        body: SafeArea(
          child: _networkConfig == null
              ...
              : _GameView(
                  networkConfig: _networkConfig!,
                  startInPracticeMode: _startInPracticeMode,
                ),
        ),
      ),
    );
...
class _GameView extends StatefulWidget {
  ...
  final bool startInPracticeMode;

  const _GameView({
    required this.networkConfig,
    required this.startInPracticeMode,
  });
...
class _ConfigurationScreen extends StatefulWidget {
  final void Function(NetworkConfig config, bool startInPracticeMode) onStart;
...
class _ConfigurationScreenState extends State<_ConfigurationScreen> {
  ...
  void _startGame({required bool startInPracticeMode}) {
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  TextField(
                    ...
                    onSubmitted: (_) => _startGame(startInPracticeMode: false),
                  ),
                  const SizedBox(height: 20),
                  ElevatedButton(
                    onPressed: () => _startGame(startInPracticeMode: false),
                    child: const Text('Start Multiplayer'),
                  ),
                  const SizedBox(height: 10),
                  OutlinedButton(
                    onPressed: () => _startGame(startInPracticeMode: true),
                    child: const Text('Camp de proves'),
                  ),
                ],
              ),
            ),
...
class _GameViewState extends State<_GameView>
  ...
  void initState() {
    ...
    _game = GameApp(
      ...
      startInPracticeMode: widget.startInPracticeMode,
    );
    _initialize();
  }
  ...
  bool _isLetterboxedMode() {
    final currentScreen = _game.getScreen();
    return currentScreen is! PlayScreen && currentScreen is! TrainingGroundScreen;
  }
